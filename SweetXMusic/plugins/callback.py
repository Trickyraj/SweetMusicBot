...
from SweetXMusic import YouTube, app
from SweetXMusic.core.call import SweetX
from SweetXMusic.misc import SUDOERS, db
from SweetXMusic.utils import bot_sys_stats
from SweetXMusic.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from SweetXMusic.utils.formatters import seconds_to_min
from SweetXMusic.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from SweetXMusic.utils.stream.autoclear import auto_clean
from SweetXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await SweetX.pause_stream(chat_id)
        ...
        await SweetX.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await SweetX.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await SweetX.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await SweetX.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await SweetX.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
